...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    out = torch.cat([out_left, out_right], dim=0)  # [2*N, D] #out_left와 out_right의 병합
    
    total_loss = 0
    for k in range(N):  # loop through each positive pair (k, k+N)
        z_k, z_k_N = out[k], out[k+N] #각각 (D,)
        ...
        exp = lambda x,y : torch.exp(sim(x,y)/tau)

        sims_k,sims_k_N =0,0
        for j in range(2*N):
          if j != k and j != k+N:
            sims_k += exp(z_k,out[j])
            sims_k_N += exp(z_k_N,out[j])

        sim_between = exp(z_k,z_k_N)

        l_k = -torch.log(sim_between/(sims_k+sim_between))
        l_k_N = -torch.log(sim_between/(sims_k_N+sim_between))
...
def compute_sim_matrix(out):
    ...
    out_norm = out/norm #vector 단계에서 정규화
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = exponential.masked_select(mask).view(2 * N, -1)  # [2*N, 2*N-1] #k=i 일 경우 제외
    
    # Hint: Compute the denominator values for all augmented samples. This should be a 2N x 1 vector.분모 계산
    ...
    sim_pairs = torch.tile(sim_positive_pairs(out_left,out_right),(2,1)) #(2N,1)
