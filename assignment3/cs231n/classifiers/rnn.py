...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        captions_in = captions[:, :-1] #마지막 word는 제외, rnn의 input
        captions_out = captions[:, 1:] #시작 word는 제외, rnn의 desired output
        ...
        h, rnn_cache = recurrent_forward(x,h0, Wx, Wh, b)
        ...
        dx, dh0, dWx, dWh, db =recurrent_backward(dh, rnn_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        input_words = np.repeat(self._start,N) #모든 sequence에서 시작 token이 start니까!

        c = np.zeros_like(h) #lstm에서만 사용!
        for t in range(max_length):
          x, _ = word_embedding_forward(input_words, W_embed)
          if self.cell_type == 'rnn':
          ...
          input_words = np.argmax(out,axis=1)
