...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        - word_to_idx: A dictionary giving the vocabulary. It contains V entries.
          and maps each string to a unique integer in the range [0, V). #각 단어를 특정한 index로 변환
        - input_dim: Dimension D of input image feature vectors. #D : 1개의 image에 대한 feature 갯수
        - wordvec_dim: Dimension W of word vectors.
        - num_heads: Number of attention heads.
        - num_layers: Number of transformer layers. #transformer layer가 몇개인지
        - max_length: Max possible sequence length. #문장의 최대 길이
        """
        super().__init__()

        vocab_size = len(word_to_idx) #predefined word가 몇개인가
    ...
    def forward(self, features, captions):
        ...
        x = self.positional_encoding(self.embedding(captions))
        proj_image = self.visual_projection(features).unsqueeze(1) 
        # features : (N,D) -> (N,W) -> (N,1,W)

        tgt_mask = torch.tril(torch.ones(T,T, device=x.device))

        out = self.transformer(x, proj_image, tgt_mask)
        #x : encoder 쪽의 정보
        #proj_image : decoder의 input
        #tgt_mask : decoder에서 attention할때 미래의 정보 사용 방지
    ...
    def sample(self, features, max_length=30):
        ...
        with torch.no_grad():
            ...
            for t in range(max_length):
                ...
                # torch.cat : dim의 방향으로 torch들을 concat
