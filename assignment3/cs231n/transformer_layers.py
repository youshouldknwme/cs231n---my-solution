...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        j = torch.arange(0,embed_dim,2).unsqueeze(0) #(1, embed_dim//2)
        exp = torch.exp(-math.log(10000)*j/embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        e = Q@K.moveaxis(2,3)/math.sqrt(self.head_dim) #(N,H,S,T)

        if attn_mask is not None:
          e = e.masked_fill(attn_mask==0, float("-inf")) #mask가 0인 지점을 음의 무한대로 만들어 softmax 계산에서 제외되도록 함

        Y = self.attn_drop(F.softmax(e,dim=-1))@V #(N,H,S,E//H)
