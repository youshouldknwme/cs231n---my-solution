...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(x.dot(Wx)+prev_h.dot(Wh)+b) #activation function 고려
...
def rnn_backward(dh, cache):
    ...
    dx = []
    dWx, dWh, db,dh0 = 0,0,0,0
    for t in range(dh.shape[1]-1,-1,-1):
      t_dx, dh0, t_dWx, t_dWh, t_db = rnn_step_backward(dh[:,t,:]+dh0,cache[t])
      dx.append(t_dx)
      dWx += t_dWx
      dWh += t_dWh
      db += t_db     
...
def word_embedding_backward(dout, cache):
    ...
    x,W = cache
    dW = np.zeros_like(W)
    np.add.at(dW,x,dout) #no return 값 
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a_i, a_f, a_o, a_g = np.split(a,4, axis=1) #각각 (N,H)
    next_c = sigmoid(a_f)*prev_c + sigmoid(a_i)*np.tanh(a_g)
    next_h = sigmoid(a_o)*np.tanh(next_c)
    cache = (x, prev_h, prev_c,next_c, a_i, a_f, a_o, a_g, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, next_c, a_i,a_f,a_o,a_g, Wx, Wh = cache
    dnext_c_plus = dnext_h*sigmoid(a_o)*(1-np.tanh(next_c)**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
      next_h,next_c, t_cache = lstm_step_forward(x[:,t,:],h[t],next_c,Wx,Wh,b)
      h.append(next_h)
      cache.append(t_cache)
...
def lstm_backward(dh, cache):
    ...
    dx = []
    dWx, dWh, db, dh0 = 0,0,0,0
    dnext_c = 0         #가장 마지막 dnext_c는 divergence에 직접적 영향 x
    for t in range(dh.shape[1]-1,-1,-1):
     t_dx,dh0,dnext_c,t_dWx, t_dWh, t_db = lstm_step_backward(dh[:,t,:]+dh0, dnext_c, cache[t])
     dx.append(t_dx)
     dWx += t_dWx
     dWh += t_dWh
     db += t_db
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    (Null token이 있는 경우는 loss를 계산하지 않는다)
    ...
    loss = -np.sum(mask_flat * np.log(probs[np.arange(N * T), y_flat])) / N #mask 되어있는 경우는 loss에 포함하지 않는다!
    dx_flat = probs.copy()
